        ...
        if(ntext[j].node_nr==name[i])
	{
          ...
          if(mode==GL_SELECT)
	  {
	      ...
	      //else
            ...
            glEnd();
            text( ntext[j].mx, ntext[j].my, ntext[j].mz, ntext[j].string, glut_font[draw_font] );
            glPopName();
            glLoadName(-1);
            continue;
	  }
          ...
          if(ntext[j].tx>0)
	  {
	    ...
	    if((int)ntext[j].tx+strlen(ntext[j].string)*pixPerCharx[draw_font]/2 < wx)
	    {
              wxl=ntext[j].tx+strlen(ntext[j].string)*pixPerCharx[draw_font];
            }
            else wxl=ntext[j].tx;   
...
void drawElements_plot( int num, int *name, Nodes *node, double *colNr, Elements *e_enqire, int col, char type, int width, int pickflag )
{
  ...
  if((type=='x')||(type=='b'))
  {
    if(width>0) { alpha=(float)width/100.; }
    else alpha=BLEND_ALPHA;
    width=1;
    if(type=='x') type='v';
  }
  else
  {
    ...
    if ( ipuf[0] == GL_FRONT ) glEnable ( GL_CULL_FACE );
    else glDisable ( GL_CULL_FACE );
  }
  glLineWidth(width);

  if((type=='v')&&(!pickflag)&&(illumResultFlag))
  {
    /* enable all colors */
    defineColTextur_load(alpha);
    //glColor3d( 1,1,1);
    glEnable(GL_TEXTURE_1D);
    ...
    for (i=0; i<num; i++ )
    {
      switch(e_enqire[name[i]].type)
      {
        ...
        glBegin ( GL_TRIANGLES );
          ...
          glNormal3dv ( e_enqire[name[i]].side[3] );
          GLCOLOR     ( colNr[e_enqire[name[i]].nod[0]] );
          glVertex3dv ( &node[e_enqire[name[i]].nod[00]].nx );
          GLCOLOR     ( colNr[e_enqire[name[i]].nod[15]] );
          glVertex3dv ( &node[e_enqire[name[i]].nod[15]].nx );
          GLCOLOR     ( colNr[e_enqire[name[i]].nod[9]] );
          glVertex3dv ( &node[e_enqire[name[i]].nod[9]].nx );
  ...
  else
  {
	...
	switch(e_enqire[name[i]].type)
	{
        ...
        x/= j; y/= j; z/= j;
        sprintf( buffer, "%d ", name[i]);
        glColor3f ( entitycol[0].r, entitycol[0].g, entitycol[0].b  );
        glDisable( GL_LIGHTING );
        text( x, y, z, buffer, glut_font[draw_font] );
        glEnable( GL_LIGHTING );
      }
    ...
    */
    ...
    for (i=0; i<num; i++ )
    {
      switch(face[name[i]].type)
      {
    	...
    	case 11:
	//glNormal3dv ( e_enqire[face[name[i]].elem_nr].side[0] );
        glDisable( GL_LIGHTING );
        glColor3f ( entitycol[col].r, entitycol[col].g, entitycol[col].b  );
        glBegin ( GL_LINES );
         glVertex3dv ( &node[face[name[i]].nod[0]].nx );
...
void drawLines_plot( int num, int *name, Lines *line , Points *point, int col, char type, int width)
{
  ...
  if(anz_p)  drawPoints_plot( anz_p, pnt_indx, point, col, 0, width+4 );
}
