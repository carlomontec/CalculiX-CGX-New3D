...
#define     MAX_REFINEMENT_LOOPS     0     // nr of loops to adjust line divisions on element edges
...
/* default location of the help files, please insert the actual one */
  ...
  #else
    #define     HELPFILE        {"/yaprod/cae/application/CalculiX/doc/cgx_2.23/cgx.html",\
                             "/yaprod/cae/application/CalculiX/doc/ccx_2.23/ccx.html"}
  #endif
  /* default tet mesher, 0:Netgen ng_vol, 1:tetgen */
  #define     TETMESHER  0
#elif defined AFLIB
  #define     HELPFILE        {"/usr/local/CalculiX/cgx_2.23/doc/cgx/cgx.html",\
    "/usr/local/CalculiX/ccx_2.23/doc/ccx/ccx.html",\
...
#define     EDGES         "+EDGE"   /* all model edge nodes */
...
#define     ORI           "-ORI"    /*  */
#define     PENETRATION   "+PENTR"  /* stores nodes of one mesh which penetrate a second mesh */
...
#define     WARNINGS      "-WARN"   /* problematic surfaces during 2d meshing */
#define     ZAP           "-ZAP"    /* entities to be deleted */
...
int readDynaList(char *datin, Summen *apre, Sets **sptr, Nodes **nptr, Elements **eptr, Datasets **lptr  );
...
void calcAnimation( int anim_steps, double anim_faktor, int *anim_alfa, int halfperiode, int centerNode, Summen *anz, Nodes *node, Elements *e_enqire, Datasets *lcase, int lc_col, int entity, Scale *scale, char surfFlag, double *colNr, int steps );
...
int commandoInterpreter( char *type, char **ptr_string, int na, int nb, FILE *handle1, int addFlag, int *flag );
...
int readccx(char *datin, Summen *anz, Sets **sptr, Nodes **nptr, Elements **eptr, Datasets **lptr, int checkflag);
