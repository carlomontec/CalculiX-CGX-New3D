...
typedef struct {
  ...
  FILE  *filehandle; /* pointer to the frdfile, needed to close the file from outside */
...
typedef struct {
  ...
  double my;
  double mz;
  char   *text;          /* node related text */
  char   *string;        /* complete string   */
} Texts;
...
typedef struct {
  ...
  int axis;
  // axis=1 v is ambiguous (a given point could have vmax or vmin)
  // axis=2 u is ambiguous (a given point could have umax or umin)
